...
public class TableOrdersManager implements OrdersManager {
    public static final int TABLES_NUM = 5;
    ...
    private void checkTableNumber(int tableNumber) {
        if (tableNumber > TABLES_NUM || tableNumber <= 0)
            throw new IllegalTableNumber("Таблица с номером " + tableNumber + " не существует");
    }
    ...
    public void add(TableOrder order, int tableNumber) throws OrderAlreadyAddedException {
        ...
        if (orders[tableNumber - 1] != null)
            throw new OrderAlreadyAddedException("В таблице с номером " + tableNumber + " уже есть заказ");
    ...
    public void addItem(MenuItem item, int tableNumber) {
        checkTableNumber(tableNumber);
        if (orders[tableNumber - 1] == null)
            throw new IllegalArgumentException("В таблице под номером " + tableNumber + " нет заказов!");
